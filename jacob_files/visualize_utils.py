...
class Box3D(object):
    ...
    def __init__(self, label_file_line):
        ...
        self.t = (float(data[11]), float(data[12]), float(data[13])) # location (x,y,z) in camera coord.
...
def read_linx_label(label_filename):
    ...
    for i in range(len(objects)):
        an_object = objects[i]
        box = [an_object.t[0], an_object.t[1], an_object.t[2], an_object.l, an_object.w, an_object.h, 0]#this might be revisited, i think heading is worngly calculated....
        boxes.append(box)
...
def read_kitti_label(label_filename):
    ...
    for i in range(len(objects)):
        an_object = objects[i]
        box = [an_object.t[2], -an_object.t[0], -an_object.t[1], an_object.w, an_object.l, an_object.h, an_object.ry]#this might be revisited, i think heading is worngly calculated....
        boxes.append(box)
        labels.append(an_object.type)
    boxes = np.array(boxes)
    labels = np.array(labels)#need to convert labels into numbers
...
def read_metro_linx_label_preprocess(label_filename):
    ...
    for i in range(len(objects)):
        an_object = objects[i]
        box = [an_object.t[0], an_object.t[2], an_object.t[1], an_object.w, an_object.l, an_object.h, np.radians(an_object.ry)]#this might be revisited, i think heading is worngly calculated....
        boxes.append(box)
        labels.append(an_object.type)
...
def read_metro_linx_label_untilt(label_filename):
    ...
    for i in range(len(objects)):
        an_object = objects[i]
        box = [an_object.t[0], an_object.t[2], an_object.t[1], an_object.w, an_object.l, an_object.h, an_object.ry]#this might be revisited, i think heading is worngly calculated....
        boxes.append(box)
        labels.append(an_object.type)
        t.append(an_object.truncation)
    boxes = np.array(boxes)
    ...
    if ref_boxes is not None and len(ref_boxes) > 0:
        ...
        if ref_labels is None:
            ...
            for k in range(ref_labels.min(), ref_labels.max() + 1):
                ...
                #mask = (ref_labels == k)
                fig = draw_corners3d(ref_corners3d, fig=fig, color=cur_color, cls=ref_scores[mask], max_num=100)
